...
def sendPos(hand, wrist, arm, elbow, base, mode = 0):
    # print(hand, wrist, arm, elbow, base)
    # print('----------------------------------')
    # print "    hand:      %i \n    wrist:     %i \n    arm:       %i \n    elbow:     %i \n    base:      %i" % (hand, wrist, arm, elbow, base)
    try:
        ser.write(struct.pack('>BBBBBB', int(hand), int(
            wrist), int(arm), int(elbow), int(base), int(mode)))
        return 0
    except:
        return 1
...
def LeapRaw(handPosX, handPosYRaw, handPosZ, grab, hand_pitch):
    ...
    sendPos(hand, wrist, arm, elbow, base, 1)
